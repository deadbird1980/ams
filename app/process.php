...
if (isset($_GET['captcha']) && $_GET['captcha'] == 1) {
    ...
    $font_path = rtrim(implode(DIRECTORY_SEPARATOR, array_slice(explode(DIRECTORY_SEPARATOR, __FILE__), 0, -2)), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . basename(dirname(__FILE__)) . DIRECTORY_SEPARATOR . 'global/';
    ...
    // sets a cookie that will later be read by the form generator and used to see if user entered the correct
    // characters
...
// if we're performing a file upload
    ...
    function process() {
        ...
        // the server path where to upload the file
        $path = preg_replace(array('/\\\\/', '/\/\//'), '/', str_replace('http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST'], $_SERVER['DOCUMENT_ROOT'], $_GET['path']));
        ...
        if (isset($_FILES[$_GET['control']])) {
            ...
            $file['error'] = (!file_exists($path) ? -1 : $_FILES[$control]['error']);   // is the upload path valid?
